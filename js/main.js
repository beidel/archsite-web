...
function (
    ...
    HomeButton, LocateButton, PrintButton, SearchMenu, BasemapToggle,
    Geocoder,
    Popup,
    Legend,
    About,
    ...
    return declare("", [About, SocialLayers], {
        ...
        constructor: function () {
            ...
            this.css = {
                ...
                iconList: "icon-th-list",
                iconSearch: "icon-search-1",
                ...
                printbuttonTheme: "PrintButtonCalcite",
        ...
        _initSearch: function () {
            var searchNode = dom.byId('SearchDiv');
            if (searchNode) {
                this._mapSearch = new SearchMenu({
                    map: this.map,
                    geometryServiceUrl: this.config.geometryServiceUrl,
                    searchLayers: this.config.searchLayers
                }, searchNode);
        ...
        _init: function () {
            ...
            if (this.config.enableSearch) {
                content = '';
                content += '<div class="' + this.css.panelHeader + '">Search</div>';
                content += '<div class="' + this.css.panelContainer + '">';
                content += '<div class="' + this.css.panelPadding + '">';
                content += '<div id="SearchDiv"></div>';
                content += '</div>';
                content += '</div>';
                // menu info
                menuObj = {
                    title: 'Search',
                    label: '<div class="' + this.css.iconSearch + '"></div><div class="' + this.css.iconText + '">Search</div>',
                    content: content
                };
                // legend menu
                if (this.config.defaultPanel === 'search') {
            ...
            if (this.config.enablePrintButton) {
                this._PB = new PrintButton({
                    map: this.map,
                    theme: this.css.printbuttonTheme,
                    printUrl: this.config.printUrl
                }, 'PrintButton');
                this._PB.startup();
                // clear locate on home button
                on(this._PB, 'print', lang.hitch(this, function () {
                    if (this._LB) {
                        this._LB.clear();
                    }
        ...
        _hideLoadingIndicator: function () {
            // add loaded class
            domClass.remove(dom.byId("mapContainer"), this.css.appLoading);
        },
